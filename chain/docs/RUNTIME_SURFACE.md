...
- `Currency = Balances`, `ZkTree = ZkTree` (the same single instance the header's root commits to), `FindAuthor = QpowAuthor`, `BlockHashWindow = 256`, `MinLeafFee = 1` pool step (0.01 QNR), `CiphertextBytesPerFeeQuantum = 512`, `FeeBurnRate = 50%`, `MaxCiphertextBytes = 2048`, `MaxOutputsPerBlock = 2048`, `CiphertextRetentionBlocks = 0` (the constant is published so that a wallet reading the metadata is told the note ciphertexts live in block bodies; nothing in the runtime reads it).
...
- **Storage:** `UsedNullifiers` (`Blake2_128Concat`, prover-chosen keys), `LeafBlocks` (`Identity`), `EntryCount`, `PoolValue` (planck the pool stands behind), `PendingCoinbase` (the payload from this block's inherent, killed at `on_initialize` and taken by the mint), `PendingCoinbaseFee` (the author's share of settled fees, waiting for a coinbase note), `CoinbaseValues` (`Identity`, a coinbase note's public value in pool steps; presence is what marks a leaf a coinbase).
...
- **Note ciphertexts are in no storage item.** They ride in the block bodies, inside the `submit_private_batch`, `submit_public_batch` and `shield` calls that append the leaves, and the events publish their lengths (`SlotSettled.ciphertext_bytes`, `Shielded.ciphertext_bytes`) rather than the payloads. A wallet reads them out of `chain_getBlock` and authenticates them against the header's `extrinsicsRoot`, which is `LayoutV0<BlakeTwo256>::ordered_trie_root` while `system_version` is 1: that field is therefore a consensus constant every wallet depends on. `docs/AUTHENTICATED_READS.md` carries the rule and `crates/qnero-state-proof` the recomputation.
