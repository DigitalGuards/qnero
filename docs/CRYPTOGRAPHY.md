...
| Ordinary note delivery | ML-KEM-1024 followed by ChaCha20-Poly1305. The vendored `qnero-pqcrypto::note_encryption` KDF derives separate note/memo material from the shared secret, labels and suite; version, suite and diversifier are authenticated. Ciphertext version 1, suite 1. Wallets pad ordinary outputs to 1792 bytes, and settlement requires exactly that: a settled ciphertext is the length its declared suite fixes, or the position is emptied whole. The 2048-byte payload cap still bounds a `shield` entry note and the encoded length. Fresh encapsulation randomness is required for every output. |
