...
6. **A wallet still takes one per-leaf value from storage unbound, and the next
   wallet milestone is to stop.** `Shielded::Ciphertexts(i)` is tied to leaf
   `i` by nothing on chain, so a node with honest headers substitutes one and
   an incoming payment is skipped in silence; `docs/WALLET.md` carries the
   bound under "What a lying node can and cannot do" and both wallets print the
   rescan that recovers it. The closure is to take every per-leaf value from
   the block body instead: read each block's extrinsics, recompute the header's
   `extrinsicsRoot` as the ordered trie root under blake2-256 and compare it
   against the header the walk already trusts, take a settlement's ciphertexts
   from the body and bind them to the tree-authenticated commitments through
   the settlement proof's public inputs, which the wallet re-verifies with
   `qnero-verifier`, and take a shield's ciphertext from its call arguments the
   same way. The price is one `chain_getBlock` per block of a scanned range
   beside the header already fetched, one trie root per block, and one
   private-batch verify per settlement at 3.7 ms native and 14.1 ms in wasm
   (`docs/BENCH.md`), so a first sync of a long chain pays it per block rather
   than per note. The reward is that bound A covers every per-leaf value rather
   than every one but this, and the rescan hint stops being the answer to a
   payment that did not arrive.
