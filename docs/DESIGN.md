...
### 7.5 Side-branch admission (client policy, 2026-09-21)

**The problem.** Genesis is the only irreversible block and the chain is chosen
by cumulative work, so a node imports, executes and archives every valid block
on every branch. A side branch retargets from its own timestamps, and the
retarget in `pallets/qpow` moves at most 99/2048 down per block but keeps
moving, so a branch left to itself reaches the 128 floor and mines blocks at
128 hashes each while every honest node still pays a hash, an execution and a
permanent archive entry for each of them. Nothing bounded the number of
branches or their length. A peer could also name a header on any historical
epoch and force a 256 MiB Argon2d seed fill that evicted the miner's live seed
from the two-entry cache.

**What was rejected, and why.** A depth floor (refuse a header more than two
seed epochs below best) and a hard difficulty threshold (refuse a side-branch
block below an eighth of the canonical difficulty at its height) were both
designed and both refuted before they were written. The first turns any
two-sided partition longer than the floor, and any bootstrap eclipse, into a
permanent split: a refusal in the verifier drops the peer and restarts sync,
the ancestor search compares canonical hashes only, and the same branch is
offered again from the same fork point for ever. The second refuses an honest
heavier chain whenever hashrate migrated across a partition, for the same
permanent reason. Underneath both is one fact: block by block, an attacker's
cheap branch and an honest minority partition's branch are the same object, a
low-hashrate chain whose difficulty decayed, and only cumulative work at the
tip separates them, which is invisible until the whole branch is in. The
arithmetic that seemed to make the floor safe was also wrong twice: the
timestamp drift a block may claim is 15 s, and the slack a branch has to spend
on lowering its difficulty grows with wall clock rather than being fixed by
where it roots, so a floor bounds nothing about difficulty.

**What was done instead.** The client budgets. A block on the tip is free. A
...
resident draws from a second bucket of 4 refilling at 24 an hour. An empty
bucket refuses the block through the ordinary verification-failed path and sync
offers it again later, so an honest heavier chain is admitted at the budget's
rate and can never be refused for good, and a spammer gets the burst and then
the rate, each block still costing a real hash. The seeds the node mines under
now and next sit in two pinned slots the LRU cannot evict. Both budgets are
charged in the verifier only, so the re-verification inside block import and
the node's own blocks never disagree with it. The reference difficulty is the
tip's, cached per tip: pricing a side-branch block against what the chain paid
at its own height would let a child of an old canonical block pass at ratio one
...
6. **Resolved by the relaunch bundle: a payment is read out of an
   authenticated block body, and `Shielded::Ciphertexts` no longer exists.**
   This asked for every per-leaf storage value to be read with
   `state_getReadProof` and verified against the header's own `stateRoot`.
   Section 12.9 is the decision as built, and it splits the answer in two
   against that same header.

   What was open. `Shielded::Ciphertexts(i)` was tied to leaf `i` by nothing on
   ...
   payment moved to any position the block's range allowed, across group
   ...
   them, so a node could answer a leaf count of 2 for a block that appended 8,
   serve the two node hashes as its leaves, and pass every root check under the
   honest chain's own headers.

   How it closed. Part one, against `stateRoot`, is what this question asked
   for and it is built: `ZkTree::LeafCount`, `ZkTree::Leaves(i)`,
   `Shielded::LeafBlocks(i)`, `Shielded::CoinbaseValues(i)`,
   `Shielded::EntryCount` and the `UsedNullifiers` pages are each read with a
   trie proof and verified locally, so the count, every leaf value and the
   fold's height are facts of the block. Part two is against `extrinsicsRoot`:
   the ciphertexts moved out of the state trie into the block bodies that
   always carried them, `Shielded::Ciphertexts` is removed from the runtime,
   and a wallet fetches the body of each block that appended a leaf, roots it
   against the `extrinsicsRoot` of the header the walk already rehashed, and
   trial-decrypts every payload in it. A body roots as a whole, so a dropped,
   reordered or appended extrinsic reaches a root no header carries, and there
   is no per-payload absence left to detect. The note that opens is placed at
   the leaf whose commitment it opens inside that block's folded range, which
   is what closes the position bound: where inside the block a node put the
   leaf decides nothing.

   What it cost. The 14.1 ms per-settlement verify this question priced a body
   route against does not apply, because the wallet needs the public inputs and
   the chain has already verified the proof, and the body is one a scanning
   wallet fetches anyway. The `sp-trie` wasm module stays, for part one. **No
   consensus change to the tree, and the upstream tree is untouched**, which is
   what keeps section 10's trust claim.

   What stays open after it. Bound B, unchanged: a node above the newest
   checkpoint still chooses every header field, `stateRoot` and
   `extrinsicsRoot` included, and the defence there is still the checkpoint
   fork walk. `docs/WALLET.md` carries it under "What a lying node can and
   cannot do".
7. **Whether the tree should domain-separate a child's slot and its level in
   ...
   the wallets now that question 6 has landed**, so it stays a consensus option
   that a network can take on its own merits, and no decision the wallets are
   waiting on. `tree::hash_node` sorts its four children so that a Merkle path
