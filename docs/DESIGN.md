...
6. **A wallet still takes two per-leaf values from storage unbound, and the
   next wallet milestone is to stop.** `Shielded::Ciphertexts(i)` is tied to
   leaf `i` by nothing on chain, and neither is the leaf's own index inside its
   aligned group of four: `tree::hash_node` sorts a node's children before
   hashing them, so a block's `zkTreeRoot` commits to each group's multiset and
   to no order inside it. A node with honest headers substitutes a ciphertext,
   or moves an incoming payment onto its block's coinbase position where no
   ciphertext is owed, and the payment is skipped in silence either way;
   `docs/WALLET.md` carries both under "What bound A does not cover" and both
   wallets print the rescan that recovers them. The closure is to take every
   per-leaf value from the block body instead: read each block's extrinsics,
   recompute the header's `extrinsicsRoot` as the ordered trie root under
   blake2-256 and compare it against the header the walk already trusts, then
   take **each leaf's commitment and its ciphertext together** out of the
   settlement and shield calls, in the order `pallet-shielded` appends them, so
   the index and the bytes are bound in one pass: a settlement's outputs go
   into the tree in slot order and its ciphertexts are bound to those same
   commitments through the settlement proof's public inputs, which the wallet
   re-verifies with `qnero-verifier`, and a shield's commitment and ciphertext
   are two arguments of one call. A block's leaf range is already
   header-authenticated by the fold, so a body read that reproduces that range
   in order pins which leaf each commitment is, which the root cannot. The
   price is one `chain_getBlock` per block of a scanned range beside the header
   already fetched, one trie root per block, and one private-batch verify per
   settlement at 3.7 ms native and 14.1 ms in wasm (`docs/BENCH.md`), so a
   first sync of a long chain pays it per block rather than per note. The
   reward is that bound A covers every per-leaf value rather than every one but
   these two, and the rescan hint stops being the answer to a payment that did
   not arrive.
7. **Whether the tree should domain-separate a child's slot instead of sorting
   it, decided before a testnet genesis.** This is the consensus-level
   alternative to open question 6 and it closes only the index half.
   `tree::hash_node` sorts its four children so that a Merkle path can carry
   siblings with no position beside them, and the cost of that convenience is
   that the root commits to each aligned group's multiset rather than to an
   order, which is the bound above. Hashing each child under its own slot, over
   the unsorted children in index order, puts the index in the root: a wallet
   that has checked a block's fold has then checked which leaf each commitment
   is, no body read is needed for the index half, and a node cannot permute a
   group at all. The cost is a fork and it is not small: the pallet's node rule
   and its proof shape, the circuit's Merkle gadget, which drops the
   sorted-sibling adapter and gains a position witness per level, both wallets'
   local rebuilds and path checks, the explorer's tree views, and every KAT
   vector over a root or a path, regenerated and reviewed. It also ends this
   tree's status as the audited upstream's as-is, which is the argument
   `docs/DESIGN.md` section 10 rests the trust claim on, so the change would
   need its own review. Decide it before a network launches, because after that
   it is a hard fork rather than a constant.
